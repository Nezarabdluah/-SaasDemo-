...
## 🤖 برومبتات استراتيجية المشروع (ABP Framework Workflows)

### 1. إعداد كيان جديد (Entity Creation)
```text
أريد إنشاء Entity جديد اسمه [EntityName] في مشروع [ProjectName].
قم بإنشاء الملف في `aspnet-core/src/[ProjectName].Domain/[PluralName]/[EntityName].cs`.
الخصائص المطلوبة:
- [Property 1] (Type)
- [Property 2] (Type)
التزم فوراً بقواعد `Project Memory`: استخدم `FullAuditedAggregateRoot<Guid>`, `private set`, `static Create`, واستخدم `Check.NotNullOrWhiteSpace`. لا تقم بإنشاء أي Repository أو AppService الآن، سنترك ذلك لـ AbpHelper.
```

### 2. إصلاح أخطاء بعد توليد AbpHelper
```text
قمت بتشغيل AbpHelper وظهرت أخطاء Compilation في الـ Backend.
إليك رسائل الخطأ من الـ Output:
[Insert Error List / Output Here]
استخدم أدواتك لفحص `Entity.cs` وإزالة الـ Constructors المكررة، وفحص `AppService` للتأكد من وجود مساحات الأسماء كـ `Localization` أو إصلاح تعارض الـ CmsKit.
```

### 3. بناء وتخصيص واجهات Angular
```text
قام AbpHelper بتوليد الـ CRUD للـ Backend.
الـ APIs المتاحة لـ [EntityName] هي GET/POST/PUT/DELETE على المسار `/api/app/[slug]`.
المطلوب إنشاء 3 صفحات Angular Standalone:
1. قائمة (List) مع Pagination
2. صفحة إنشاء/تعديل (Create/Edit Form)
3. صفحة عرض (Detail View)
استخدم `RestService` وأضف الـ Routes لـ `app.routes.ts` والـ Menu في `route.provider.ts`.
```
