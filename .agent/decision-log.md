...
| 2026-03-27 | `npm run build` ≠ Server build<br>*(أمر البناء لا يشمل السيرفر)* | `ng build` compiles browser only. Server needs separate script.<br>*(أمر البناء يقتصر على المتصفح، السيرفر يحتاج أمر مخصص.)* |
| 2026-04-04 | SiteSettings as Singleton Entity<br>*(كيان الإعدادات كسجل وحيد)* | One row in DB. `FirstOrDefaultAsync()` + auto-create on first update. Merges SocialLinks + EmailSettings.<br>*(سجل واحد فقط. الإنشاء تلقائي عند أول تحديث. دمج روابط التواصل والبريد.)* |
| 2026-04-04 | Added `/playwright-qa` skill<br>*(إضافة مهارة اختبار الجودة)* | Universal QA Agent for E2E testing with Playwright. Skill #29.<br>*(وكيل QA عالمي للاختبارات الشاملة — المهارة رقم 29.)* |
